...
Features:
  - Dropdown listing all saved experiment models (exp_01 to exp_10 + best + latest)
  - Live game feed (raw RGB 210x160)
  - Per-experiment score tracking
  - Comparison bar chart — all experiments scored side by side
  - Auto-detects models in logs/ folder
...
# colours per experiment slot for comparison chart
EXP_COLORS = [
    "#00e5b0","#ff3860","#8b5cf6","#fbbf24","#38bdf8",
...
def discover_models() -> list[dict]:
    ...
    Returns list of dicts: {label, path, exp_id}
    """
    models = []

    # priority models in cwd
    for fname, label in [
...
class PlayGUI:
    def __init__(self, root: tk.Tk, args):
        ...
        root.title("DemonAttack-v5  |  Experiment Selector  |  GreedyQPolicy")
    ...
    def _build(self):
        ...
        tk.Label(hdr, text="  DEMON ATTACK  |  Experiment Player",
                 font=("Courier",15,"bold"), bg=PANEL, fg=ACC2
                 ).pack(side=tk.LEFT, padx=20, pady=10)
        tk.Label(hdr, text="Select any experiment model — compare performance live",
        ...
        self.sel_combo = ttk.Combobox(sel, textvariable=self.sel_var,
        ...
        tk.Button(sel, text=" ↺ REFRESH LIST ",
                  bg="white", fg="black",
                  font=("Courier",9,"bold"), relief=tk.FLAT,
                  padx=8, cursor="hand2",
                  activebackground="#dddddd", activeforeground="black",
        ...
        self.run_btn = tk.Button(sel, text=" STOP ",
                                 bg=ACC2, fg="black",
                                 font=("Courier",9,"bold"), relief=tk.FLAT,
                                 padx=10, cursor="hand2",
        ...
        tk.Button(sel, text=" RESET SCORES ",
                  bg="white", fg="black",
                  font=("Courier",9,"bold"), relief=tk.FLAT,
                  padx=8, cursor="hand2",
                  activebackground="#dddddd", activeforeground="black",
        ...
        for lbl, key, col in [
            ...
            tk.Label(f, text=lbl, font=("Courier",6,"bold"),
                     bg=CARD, fg=MUTED).pack(anchor=tk.W)
            var = tk.StringVar(value="--")
            tk.Label(f, textvariable=var,
                     font=("Courier",10,"bold"), bg=CARD, fg=col
                     ).pack(anchor=tk.W)
            self.sv[key] = var
        ...
        tk.Label(cf, text="LIVE FEED  |  210×160 RGB",
                 font=FT, bg=CARD, fg=ACC1).pack(anchor=tk.W, padx=8, pady=(4,0))
        ...
        tk.Label(ec, text="CURRENT MODEL — EPISODE SCORES",
                 font=FT, bg=CARD, fg=ACC1).pack(anchor=tk.W, padx=8, pady=(4,0))
        self.ep_fig, self.ep_ax = plt.subplots(figsize=(7,1.4), facecolor=CARD)
        ...
        tk.Label(right, text="EXPERIMENT COMPARISON",
                 font=("Courier",9,"bold"), bg=BG, fg=ACC1
                 ).pack(anchor=tk.W, padx=8, pady=(0,4))
        tk.Label(right,
                 text="Each bar = mean of last 10 episodes\nfor that experiment.",
                 font=("Courier",7), bg=BG, fg=MUTED, justify=tk.LEFT
        ...
        tk.Label(sb_f, text="SCOREBOARD  (last 10 eps mean)",
                 font=("Courier",7,"bold"), bg=CARD, fg=ACC1
                 ).pack(anchor=tk.W, padx=8, pady=(6,2))
        self.score_text = tk.Text(sb_f, height=12, bg=DARK, fg=TEXT,
    ...
    def _redraw_ep_chart(self):
        ...
        if rewards:
            ...
            self.ep_ax.set_title(self.current_label[:50],
            ...
            while self.running:
                ...
                with self._switch_lock:
                    if self._switch_model_path is not None:
                        ...
                        self.status_var.set(
                            f"Switched → {current_label}  |  GreedyQPolicy")
                ...
                while not done and self.running:
                    ...
                    self.sv["act"].set(ACTION_NAMES[int(action[0])])
                    self.sv["rew"].set(f"{ep_r:.0f}")
                ...
                if self.running:
                    ...
                    mean_last = np.mean(self.ep_rewards[-10:])
                    self.status_var.set(
                        f"{current_label[:40]}  |  "
                        f"ep={self.play_ep}  score={ep_r:.0f}  "
                        f"best={best:.0f}  mean10={mean_last:.0f}")
